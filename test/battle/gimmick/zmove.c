...
SINGLE_BATTLE_TEST("(Z-MOVE) Z-Moves do not retain priority")
{
    GIVEN {
        ...
        ASSUME(GetMoveType(MOVE_QUICK_ATTACK) == TYPE_NEUTRAL);
...
{
    GIVEN {
        ASSUME(GetMoveType(MOVE_CELEBRATE) == TYPE_NEUTRAL);
...
SINGLE_BATTLE_TEST("(Z-MOVE) Z_EFFECT_BOOST_CRITS raises a battler's critical hit ratio by 2 stages")
{
    ...
    GIVEN {
        ...
        ASSUME(GetMoveType(MOVE_FORESIGHT) == TYPE_NEUTRAL);
...
SINGLE_BATTLE_TEST("(Z-MOVE) Z_EFFECT_CURSE activates Z_EFFECT_RECOVER_HP or Z_EFFECT_ATK_UP_1 depending on the type of the battler")
{
    ...
    GIVEN {
        ...
        ASSUME(GetSpeciesType(SPECIES_WOBBUFFET, 0) != TYPE_UNDEAD);
        ASSUME(GetSpeciesType(SPECIES_WOBBUFFET, 1) != TYPE_UNDEAD);
        ASSUME(GetSpeciesType(SPECIES_DUSCLOPS, 0) == TYPE_UNDEAD);
...
{
    GIVEN {
        ASSUME(GetMoveType(MOVE_COPYCAT) == TYPE_NEUTRAL);
...
SINGLE_BATTLE_TEST("(Z-MOVE) Z-Me First raises the user's speed by two stages and copies the last used non-status move as a Z-Move with boosted damage", s16 damage)
{
    ...
    GIVEN {
        ASSUME(GetMoveType(MOVE_ME_FIRST) == TYPE_NEUTRAL);
...
SINGLE_BATTLE_TEST("(Z-MOVE) Z-Nature Power transforms into different Z-Moves based on the current terrain")
{
    ...
    PARAMETRIZE { terrainMove = MOVE_PSYCHIC_TERRAIN;   zMove = gTypesInfo[TYPE_LIGHT].zMove; }
    ...
    GIVEN {
        ASSUME(GetMoveType(MOVE_NATURE_POWER) == TYPE_NEUTRAL);
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_NORMALIUM_Z); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
SINGLE_BATTLE_TEST("(Z-MOVE) Z-Hidden Power always transforms into Breakneck Blitz")
{
    ...
    PARAMETRIZE { iv = 31; }
    GIVEN {
        ASSUME(GetMoveType(MOVE_HIDDEN_POWER) == TYPE_NEUTRAL);
...
{
    GIVEN {
        ASSUME(GetMoveType(MOVE_SLEEP_TALK) == TYPE_NEUTRAL);
...
SINGLE_BATTLE_TEST("(Z-MOVE) Z-Revelation Dance always transforms into Breakneck Blitz")
{
    ...
    GIVEN {
        ASSUME(GetMoveType(MOVE_REVELATION_DANCE) == TYPE_NEUTRAL);
