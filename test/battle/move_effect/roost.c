...
ASSUMPTIONS
{
    ...
    ASSUME(GetMoveType(MOVE_STEEL_WING) == TYPE_METAL);
...
{
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_SKARMORY, 0) == TYPE_METAL);
        ASSUME(GetSpeciesType(SPECIES_SKARMORY, 1) == TYPE_WIND);
...
SINGLE_BATTLE_TEST("Roost, if used by a Mystery/Flying type, treats the user as a Mystery/Mystery type until the end of the turn")
{
    ...
    GIVEN {
        ...
        ASSUME(GetSpeciesType(SPECIES_MOLTRES, 1) == TYPE_WIND);
...
{
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_KECLEON, 0) != TYPE_WIND);
        ASSUME(GetSpeciesType(SPECIES_KECLEON, 1) != TYPE_WIND);
...
{
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_RAYQUAZA, 1) == TYPE_WIND);
