...
{
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_STEELIX, 0) == TYPE_MACHINE);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Toxic Spikes are removed by grounded Poison-type Pokémon on switch in")
{
    ...
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_EKANS, 0) == TYPE_FILTH);
        ASSUME(GetSpeciesType(SPECIES_ZUBAT, 0) == TYPE_FILTH);
        ASSUME(GetSpeciesType(SPECIES_ZUBAT, 1) == TYPE_WIND);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(species) { Item(item); }
    } WHEN {
