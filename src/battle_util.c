...
u32 IsAbilityPreventingEscape(u32 battler)
{
    ...
    for (u32 battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
    {
        ...
        if (ability == ABILITY_MAGNET_PULL && IS_BATTLER_OF_TYPE(battler, TYPE_MACHINE))
            return battlerDef + 1;
    }
...
bool32 CanSetNonVolatileStatus(u32 battlerAtk, u32 battlerDef, u32 abilityAtk, u32 abilityDef, enum MoveEffect effect, enum FunctionCallOption option)
{
    ...
    case MOVE_EFFECT_TOXIC:
        ...
        else if (abilityAtk != ABILITY_CORROSION && IS_BATTLER_ANY_TYPE(battlerDef, TYPE_FILTH, TYPE_MACHINE))
        {
...
static inline s32 DoFutureSightAttackDamageCalcVars(struct DamageContext *ctx)
{
    ...
    if (GetSpeciesType(partyMonSpecies, 0) == moveType || GetSpeciesType(partyMonSpecies, 1) == moveType || gSpeciesInfo[partyMonSpecies].types[2] == moveType)
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct DamageContext *ctx, uq4_12_t modifier)
{
    ...
    else if (ctx->moveType == TYPE_EARTH && !IsBattlerGroundedInverseCheck(ctx->battlerDef, INVERSE_BATTLE, CHECK_IRON_BALL) && !(MoveIgnoresTypeIfFlyingAndUngrounded(ctx->move)))
    {
        modifier = UQ_4_12(0.0);
