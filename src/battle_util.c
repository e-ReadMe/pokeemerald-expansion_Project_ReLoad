...
uq4_12_t GetOverworldTypeEffectiveness(struct Pokemon *mon, enum Type moveType)
{
    ...
    enum Type type1 = GetSpeciesType(speciesDef, 1);
    enum Type type2 = GetSpeciesType(speciesDef, 2);//checks types 2 and 3 because type 1 will be the attribute and not used for damage calc
