...
//commented-out this section as i think its redundant by "u32 typeEffectiveness = GetBattleMonTypeMatchup", but i'm keeping it now just in case. 
...
//----------------------------------------------------------
    ...
    u8 atkType1 = opposingBattleMon.types[1], atkType2 = opposingBattleMon.types[2],//only checks types 2 and 3, ignores the first type because its the attribute which isn't involved in teh calculation
    defType1 = battleMon.types[1], defType2 = battleMon.types[2];//only checks types 2 and 3, ignores the first type because its the attribute which isn't involved in teh calculation
