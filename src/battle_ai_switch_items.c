...
bool32 IsMonGrounded(enum HoldEffect heldItemEffect, enum Ability ability, enum Type type1, enum Type type2, enum Type type3)
{
...
static u32 GetSwitchinHazardsDamage(u32 battler, struct BattlePokemon *battleMon)
{
    enum Type defType1 = battleMon->types[0], defType2 = battleMon->types[1], defType3 = battleMon->types[2];
    u8 tSpikesLayers;
        ...
        !((gFieldStatuses & STATUS_FIELD_MAGIC_ROOM) || ability == ABILITY_KLUTZ)))
    {
        ...
        // Spikes
        if (IsHazardOnSide(side, HAZARDS_SPIKES) && IsMonGrounded(heldItemEffect, ability, defType1, defType2, defType3))
        {
        ...
        if (IsHazardOnSide(side, HAZARDS_TOXIC_SPIKES) && (defType1 != TYPE_FILTH && defType2 != TYPE_FILTH
...
{
    enum Type defType1 = gAiLogicData->switchinCandidate.battleMon.types[0], defType2 = gAiLogicData->switchinCandidate.battleMon.types[1], defType3 = gAiLogicData->switchinCandidate.battleMon.types[2];
