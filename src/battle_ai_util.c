...
void SaveBattlerData(u32 battlerId)
{
    ...
    gAiThinkingStruct->saved[battlerId].types[2] = gBattleMons[battlerId].types[2];
}
    ...
    if (!BattlerHasAi(battlerId) && gAiThinkingStruct->saved[battlerId].saved)
    {
        ...
        if (illusionSpecies != SPECIES_NONE && ShouldFailForIllusion(illusionSpecies, battlerId))
        {
                ...
                && gBattleMons[battlerId].types[2] == GetSpeciesType(species, 2))
            {
                ...
                gBattleMons[battlerId].types[2] = GetSpeciesType(illusionSpecies, 2);
            }
...
void RestoreBattlerData(u32 battlerId)
{
    ...
    gBattleMons[battlerId].types[2] = gAiThinkingStruct->saved[battlerId].types[2];
}
...
bool32 IsBattlerTrapped(u32 battlerAtk, u32 battlerDef)
{
    ...
    if (AI_IsAbilityOnSide(battlerAtk, ABILITY_MAGNET_PULL)
        && IS_BATTLER_OF_TYPE(battlerAtk, TYPE_METAL))
        return TRUE;
...
static bool32 PartyBattlerShouldAvoidHazards(u32 currBattler, u32 switchBattler)
{
    ...
    enum Type type3 = GetSpeciesType(species, 2);
    ...
    if (IsHazardOnSide(side, HAZARDS_SPIKES) && ((type1 != TYPE_WIND && type2 != TYPE_WIND && type3 != TYPE_WIND
